...
void check_touch_key(U16 key) {
	static U8   *tk_addr = TOOL_DEFINE_ADDRESS_RAWCNT;
	static U8   tk_num = 0;
	static U8   selected_pin = LED1;

	if(key != 0) {
		if(key & KEY_PWRSW) { 
			tk_num = 16; 
		}
		else if(key & KEY_PAD1) { 
			tk_num = 15;
			selected_pin = LED1;
		}
		else if(key & KEY_PAD2) { 
			tk_num = 14;
			selected_pin = LED2;
		}
		else if(key & KEY_PAD3) { 
			tk_num = 13;
			selected_pin = LED3;
		}
		...
		drv_set_gpio(selected_pin | GPIO_PUSH_PULL | GPIO_ACTIVE_HIGH);
	}
	pwm_freq = (*(tk_addr + (tk_num << 1) + 1));
//	pwm_freq = ((*(tk_addr + (tk_num << 1) + 1)) | (*(tk_addr + (tk_num << 1)) & 0xff));
...
//	Cust_I2C_SetAddress(I2C_ADD_START);
    ...
    GPIOIE3 &= ~0x1; // Disable GPIO24 INPUT mode
	GPIOOE3 |=  0x1; // Enable GPIO24 OUTPUT mode
	GPIODO3 |=  0x1; // Set GPIO24 output LOW
